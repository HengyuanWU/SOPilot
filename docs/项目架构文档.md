...
2. **生命周期管理** ✅
   ...
   def register_lifecycle(app: FastAPI) -> None:
       @app.on_event("startup")  # ✅ 启动事件
       async def on_startup() -> None:
           # 初始化数据库连接
           init_neo4j(settings)
           install_schema()
       
       @app.on_event("shutdown")  # ✅ 关闭事件
...
3. **应用工厂模式** ✅
   ...
   def create_app() -> FastAPI:
       setup_logging()
       app = FastAPI(title="SOPilot API", version="0.1.0")  # ✅ OpenAPI 配置
...
| startup/shutdown 事件 | ✅ | `core/lifecycle.py` | ✅ 完全符合 |
